...
import com.doctoraak.doctoraakdoctor.databinding.ActivityLabProfileBinding
...
class LabProfileActivity : BaseActivity()
{
    private val PERMISSION_READ_EXTERNAL_STORAGE_PROFILE_CODE = 45
    private val PICK_PROFILE_IMAGE_REQ: Int = 6

    private lateinit var binding: ActivityLabProfileBinding
    private val viewModel by lazy {
        ViewModelProviders.of(this).get(LabProfileViewModel::class.java)
    }
    private val areaAdapter by lazy { SpinnerAdapter(this@LabProfileActivity) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_lab_profile)
        setToolBar(binding.toolBarProfile.toolBar)

        binding.lifecycleOwner = this
        binding.clickHandler = LabProfileClickHandler()
    ...
    private fun initViews()
    {
        ...
        binding.swDelivery.isChecked = viewModel.lab.value?.delivery != 0

        viewModel.lab.value?.insuranceList?.let {
            binding.etInsurance.setText(getLabInsuranceNames(it))
            viewModel.lab.value?.insuranceIds = getLabInsuranceIds(it)
        }

        city.forEachIndexed { index, city ->
            if (viewModel.lab.value?.city_id == city.id)
            {
        ...
        Log.d("saif", "${viewModel.lab.value?.area}")

        areaAdapter.list.forEachIndexed { index, area ->
            if (viewModel.lab.value?.area_id == area.id)
            {
        ...
        viewModel.lab.value?.workingHours?.let {
            binding.etWorkingHours.setText(
                Utils.getWorkingDays(this@LabProfileActivity, it)
            )
        }
    }
    ...
    private fun observeData()
    {
        ...
        viewModel.updateProfile.observe(this, Observer {
            ...
            it.data?.let {
                SessionManager.logIn(UserType.LAB, it)
            }
        })
    }
    ...
    inner class LabProfileClickHandler
    {
        fun onUpdateProfile()
        {
            ...
            val isEmail = lab.email.validateEmail(binding.etlEmail)
            val isArea = lab.area_id.validateId(binding.splArea, getString(R.string.area_is_required))
            val isAddress = lab.lang.validateEmpty(lab.latt, binding.etlAddress, getString(R.string.pick_address))
            val isWorkingHours = if (lab.workingHours.isEmpty()) {
                binding.etlWorkingHours.error = getString(R.string.select_your_working_hours)
                false
            } else true

            if (isWorkingHours && isCity && isArea && isAddress && isEmail && isPhone)
            {
                Log.d("saif", "onUpdateProfile: lab= " + lab.toString())
                showWarning(this@LabProfileActivity, getString(R.string.are_you_sure_you_want_to_update_profile_)
                    , okClick = {  viewModel.updateProfileLab(lab) })
            }
        }

        fun onProfileImage() { requestReadExternalPermission(
            { pickImageFromGallery() },
        ...
        fun onWorkingHours() = WorkingHoursDialog.newInstance(
            this@LabProfileActivity, workingHoursList = viewModel.lab.value?.workingHours
                ...
                override fun onWorkingHoursSelected(workingHours: List<WorkingHour>) {
                    viewModel.lab.value?.let {
                        ...
                        binding.etWorkingHours.setText(Utils
                            .getWorkingDays(this@LabProfileActivity, workingHours))
                    }
                }
            }).show()
